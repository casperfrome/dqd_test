...
def count_facts(connection: sqlite3.Connection) -> int:
    row = connection.execute("SELECT COUNT(*) AS count FROM ai_database_facts").fetchone()
    return int(row["count"])


def get_latest_fact_updated_at(connection: sqlite3.Connection) -> str | None:
    row = connection.execute("SELECT MAX(updated_at) AS updated_at FROM ai_database_facts").fetchone()
...
def insert_fact(
    ...
    *,
    ...
    connection.execute(
        "INSERT INTO ai_database_fact_fts(rowid, title, content) VALUES (?, ?, ?)",
        (fact_id, title, content),
    )
    return fact_id
...
def search_facts(connection: sqlite3.Connection, match_query: str, *, limit: int) -> list[dict[str, Any]]:
    rows = fetch_all_dicts(
        ...
        SELECT
            ...
            f.content,
            f.metadata_json,
            bm25(ai_database_fact_fts) AS score
        FROM ai_database_fact_fts
        JOIN ai_database_facts f ON f.id = ai_database_fact_fts.rowid
        WHERE ai_database_fact_fts MATCH ?
        ORDER BY score
        LIMIT ?
        """,
        (match_query, limit),
...
def create_session(
    ...
    *,
    ...
    cursor = connection.execute(
        """
        INSERT INTO ai_chat_sessions (public_id, owner_user_id, client_id_hash, title, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?)
        """,
...
def get_session_by_id(connection: sqlite3.Connection, session_id: int) -> dict[str, Any] | None:
    return fetch_one_dict(connection, "SELECT * FROM ai_chat_sessions WHERE id = ?", (session_id,))


def get_session_by_public_id(connection: sqlite3.Connection, public_id: str) -> dict[str, Any] | None:
    return fetch_one_dict(connection, "SELECT * FROM ai_chat_sessions WHERE public_id = ?", (public_id,))
...
def get_message_by_id(connection: sqlite3.Connection, message_id: int) -> dict[str, Any] | None:
    row = fetch_one_dict(connection, "SELECT * FROM ai_chat_messages WHERE id = ?", (message_id,))
